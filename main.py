"""
Agente de Ventas WhatsApp - Automotora Marco Yáñez Langer
Rancagua, Chile | automotorarancagua.com

Stack cloud gratuito:
  - FastAPI en Railway (~$5/mes cubierto por crédito)
  - IA: Groq API (Llama 3.3 70B, tier gratuito, sin tarjeta)
  - Inventario: Google Sheets público o CSV local
...
GSHEETS_CSV_URL = os.getenv("GSHEETS_CSV_URL", "")
INVENTARIO_PATH = os.getenv("INVENTARIO_PATH", "inventario.csv")

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger(__name__)

app = FastAPI(title="Agente Ventas - Marco Yáñez Langer Multi-Escenario")
...
class SistemaProteccion:
    ...
    def generar_dashboard_cliente(self, cliente_id):
        ventas_cliente = [
            ...
            # Aquí filtraríamos por cliente_id en versión multi-tenant
        ]
...
# ─── SYSTEM PROMPT ────────────────────────────────────────────────────────────
SYSTEM_PROMPT = """Eres Valentina, asesora de ventas de la Automotora Marco Yáñez Langer, \
ubicada en Av. Libertador Bernardo O'Higgins 0214, Rancagua, Chile. \
Llevamos más de 53 años en el rubro. Lema: "Mi única meta es tu sonrisa."
...
async def cargar_inventario() -> pd.DataFrame:
    """
    Carga desde Google Sheets (CSV público) o CSV local.
    Google Sheets: Archivo → Publicar en la web → Hoja → CSV → copiar URL → pegar en GSHEETS_CSV_URL
    """
    if GSHEETS_CSV_URL:
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                resp = await client.get(GSHEETS_CSV_URL)
                resp.raise_for_status()
                df = pd.read_csv(StringIO(resp.text))
                log.info(f"Inventario Google Sheets: {len(df)} vehículos")
                return df
        except Exception as e:
            log.warning(f"Google Sheets falló: {e}. Usando CSV local.")

    if os.path.exists(INVENTARIO_PATH):
        df = pd.read_csv(INVENTARIO_PATH)
        log.info(f"Inventario CSV local: {len(df)} vehículos")
        return df

    log.warning("Sin inventario configurado. Usando datos de demo.")
    data = [
        ...
        {"Marca":"Fiat","Modelo":"Uno Way 1.4","Año":2017,"Combustible":"Gasolina","Kilometraje":131500,"Precio":4990000,"Stock":1,"Tipo":"Hatchback"},
        {"Marca":"Citroën","Modelo":"C3","Año":2018,"Combustible":"Diesel","Kilometraje":13000,"Precio":7200000,"Stock":1,"Tipo":"Hatchback"},
        {"Marca":"Nissan","Modelo":"Kicks","Año":2022,"Combustible":"Gasolina","Kilometraje":18000,"Precio":12800000,"Stock":1,"Tipo":"SUV"},
        {"Marca":"Kia","Modelo":"Sportage","Año":2020,"Combustible":"Gasolina","Kilometraje":52000,"Precio":11900000,"Stock":1,"Tipo":"SUV"},
        {"Marca":"Volkswagen","Modelo":"Gol","Año":2018,"Combustible":"Gasolina","Kilometraje":70000,"Precio":5800000,"Stock":1,"Tipo":"Sedán"},
    ]
    return pd.DataFrame(data)


def buscar_autos(consulta: str, df: pd.DataFrame, max_resultados: int = 3) -> str:
    """Filtra el inventario por palabras clave del mensaje del cliente."""
    q = consulta.lower()
    palabras = [p for p in q.split() if len(p) >= 3]
    cols_texto = ["Marca", "Modelo", "Tipo", "Combustible"]
    mask = pd.Series([False] * len(df))

    for palabra in palabras:
        for col in cols_texto:
            if col in df.columns:
                mask |= df[col].astype(str).str.lower().str.contains(palabra, na=False)
        if palabra.isdigit() and len(palabra) == 4 and "Año" in df.columns:
            mask |= df["Año"].astype(str) == palabra

    if any(k in q for k in ["barato","económico","economico","bajo precio","accesible"]):
        if "Precio" in df.columns:
            mask |= df["Precio"] < 7000000
    if any(k in q for k in ["suv","camioneta","pickup","4x4","todo terreno"]):
        if "Tipo" in df.columns:
            mask |= df["Tipo"].str.lower().isin(["suv","camioneta"])
    if any(k in q for k in ["diesel","diésel"]):
        if "Combustible" in df.columns:
            mask |= df["Combustible"].str.lower().str.contains("diesel", na=False)

    disponibles = df[df["Stock"] > 0] if "Stock" in df.columns else df
    resultados = disponibles[mask] if mask.any() else pd.DataFrame()

    if resultados.empty:
        resultados = disponibles.head(max_resultados)
        prefijo = "No encontré exactamente lo que buscas, pero tenemos estos disponibles:\n"
    else:
        resultados = resultados.head(max_resultados)
    ...
    for _, row in resultados.iterrows():
        precio = f"${int(row['Precio']):,}".replace(",",".") if "Precio" in row else "Consultar"
        km = f"{int(row['Kilometraje']):,} kms".replace(",",".") if "Kilometraje" in row else ""
        lineas.append(f"• {row['Marca']} {row['Modelo']} {row.get('Año','')} | {row.get('Combustible','')} | {km} | {precio}")
...
async def consultar_groq(mensaje: str, stock_context: str) -> str:
    ...
    Tier gratuito: ~500.000 tokens/día, 14.400 requests/día.
    ...
    payload = {
        "model": GROQ_MODEL,
        "messages": [
            {"role": "system", "content": SYSTEM_PROMPT.format(stock_context=stock_context)},
...
async def webhook(request: Request):
    """
    Recibe mensajes de Evolution API v2.
    Configurar en Evolution API:
      URL webhook: https://TU-APP.railway.app/webhook
    ...
    if data.get("key", {}).get("fromMe"):
        return JSONResponse({"status": "ignorado", "motivo": "propio"})

    remote_jid = data.get("key", {}).get("remoteJid", "")
    if not remote_jid or "status" in remote_jid:
        return JSONResponse({"status": "ignorado"})
    ...
    if not texto:
        return JSONResponse({"status": "ignorado", "motivo": "sin texto"})

    log.info(f"💬 {numero}: {texto}")

    df = await cargar_inventario()
    stock_context = buscar_autos(texto, df)
    respuesta = await consultar_groq(texto, stock_context)
    
    # Registrar conversación en sistema de protección
    conv_id, codigo = sistema_proteccion.registrar_conversacion(numero, texto, respuesta)
    
    # Enviar respuesta con código de seguimiento
    await enviar_whatsapp(numero, respuesta, codigo)
...
async def ver_inventario():
    df = await cargar_inventario()
    return {"total": len(df), "autos": df.to_dict(orient="records")}
...
async def atribuir_venta(request: Request):
    """Atribuir una venta al bot. Enviar JSON: 
    {"numero_cliente": "+569XXXXXXXX", "monto_venta": 9800000, "fecha_venta": "2024-01-15", "evidencias": {"codigo_mencionado": true}}"""
    body = await request.json()
...
async def detectar_fraude_cliente(request: Request):
    """Detectar posibles fraudes para un cliente. Enviar JSON: 
    {"cliente_id": "cliente1", "metrics": {"ventas_atribuidas": 5, "promedio_ventas": 15, "mensajes_bot": 0, "ventas_humanas": 10}}"""
    body = await request.json()
...
@app.get("/conversaciones/{numero_cliente}")
async def ver_conversaciones_cliente(numero_cliente: str):
    """Ver todas las conversaciones de un cliente específico."""
    conversaciones = [
        conv for conv in sistema_proteccion.conversaciones.values()
        if conv["numero_cliente"] == numero_cliente
    ]
...
@app.get("/ventas_atribuidas")
async def ver_ventas_atribuidas():
    """Ver todas las ventas atribuidas al bot."""
    return JSONResponse({
        "ventas_atribuidas": list(sistema_proteccion.ventas_atribuidas.values()),
        "total": len(sistema_proteccion.ventas_atribuidas),
        "comisiones_totales": sum(v["comision"] for v in sistema_proteccion.ventas_atribuidas.values())
    })
